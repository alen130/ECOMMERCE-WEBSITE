...
const ProductDetails = () => {
  ...
  useEffect(() => {
    if (params?.slug && !hasSpoken) {
      getProduct();
    }
  }, [params?.slug, hasSpoken]);
  ...
  const getProduct = async () => {
    try {
      ...
      setHasSpoken(true); // Set the state to true after speaking
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const speakProductIntroduction = (product) => {
    if (product) {
      return `Product Details. Name: ${product.name}. Description: ${product.description}. Price: £ ${product.price}. Category: ${product?.category?.name}.`;
    } else {
      return ''; // Return an empty string or handle the case when the product is undefined
    }
  };
